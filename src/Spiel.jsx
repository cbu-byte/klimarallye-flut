...
function Spiel({ level, onBackToDashboard, onLevelComplete }) {
  ...
  // Funktion zum Verarbeiten von Touch-Events
const handleTouchStart = (e, building) => {
  setSelectedBuilding(building);
};
...
const handleTouchMove = (e) => {
  ...
  if (target && target.dataset.zoneId) {
    setHoverZone(target.dataset.zoneId); // Zone hervorheben
  }
};

const handleTouchEnd = () => {
  if (hoverZone) {
    handleDrop(hoverZone);
    setHoverZone(null);
  }
};
      ...
      {zones.map(zone => (
        <div
          ...
          style={{
            ...
            width: '80px',
            height: '80px',
            backgroundColor: zone.occupied ? 'transparent' : 'rgba(128, 128, 128, 0.5)',
            border: zone.occupied ? 'none' : '2px solid gray',
          }}
